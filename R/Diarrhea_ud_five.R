...
load_and_process_climatedata <- function(data_path,
  ...
  for (i in 1:max_lag) {
    ...
    if (!is.null(spi_col)) spi_data[[paste0("spi_lag", i)]] <- lag(spi_data$spi, n = i, default = NA)
    if (!is.null(runoff_col)) runoff_data[[paste0("runoff_lag", i)]] <- lag(runoff_data$runoff, n = i, default = NA)
  }
...
diarrhea_do_analysis <- function(health_data_path,
  ...
  # relative ristk plot
  plot_relative_risk(combined_data$data, save_fig = save_fig, param_terms = param_terms,
                     model = a$model, level = param_level, output_dir = output_dir)
