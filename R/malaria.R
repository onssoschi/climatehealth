#' R-code for Malaria disease cases attributable to extreme precipitation
...
#'
#' @export
load_and_process_map_malaria <- function(map_path,
                                 region_col,
                                 district_col,
                                 geometry_col,
                                 output_dir = NULL){
...
#' Read in and format health data - Malaria diseases case
#'
...
#' for Malaria outcome, which may be disaggregated by sex (under five case or
...
#'
#' @export
load_and_process_data_malaria <- function(health_data_path,
                                  region_col,
                                  district_col,
                                  date_col = NULL,
                                  year_col = NULL,
  ...
  data <- data %>% rename(
    year = year_col, month = month_col, region = region_col,
    district = district_col, Malaria = Malaria_case_col,
    tot_pop = tot_pop_col) %>% select(
    all_of(c("region", "district", "year", "month", "Malaria", "tot_pop"))
  )
  return(data)
}
...
#'
#' @export
load_and_process_climatedata_malaria <- function(climate_data_path,
                                         district_col,
                                         year_col,
                                         month_col,
                                         tmin_col,
...
#'
#' @export
combine_health_climate_data_malaria <- function(health_data_path,
                                        climate_data_path,
                                        map_path,
                                        region_col,
                                        district_col,
  ...
  climate_data <- load_and_process_climatedata(climate_data_path, district_col,
                                               ...
                                               r_humidity_col, runoff_col,cvh_col,
                                               spi_col, max_lag)
...
#'
#' @export
plot_health_climate_timeseries_malaria <- function(data,
                                           param_term,
                                           level = "country",
                                           filter_year = NULL,
                                           save_fig = FALSE,
  ...
  vars_all <- c("Malaria", "tmin", "tmean", "tmax", "rainfall")
  ...
  agg <- data %>%
    ...
    summarise(across(all_of(vars_to_plot),
                     ~ if (level == "country" && "Malaria" %in% cur_column()) {
                       if (cur_column() == "Malaria") sum(.x, na.rm = TRUE) else mean(.x, na.rm = TRUE)
                     } else {
                       mean(.x, na.rm = TRUE)
                     }),
              .groups = "drop") %>%
...
#'
#' @export
set_cross_basis_malaria <- function(data) {
...
#'
#' @export
create_inla_indices_malaria <- function(data) {
...
#'
#' @export
check_vif_malaria <- function(data,
                      inla_param,
...
#'
#' @export
run_inla_models_malaria <- function(combined_data,
                            basis_matrices_choices,
                            inla_param,
                            output_dir = NULL,
                            save_csv = FALSE,
...
#'
#' @export
plot_monthly_random_effects_malaria <- function(combined_data,
                                        model,
                                        save_fig = FALSE,
                                        output_dir = NULL) {
...
plot_yearly_spatial_random_effect_malaria <- function(combined_data ,
                                              model,
                                              save_fig = FALSE,
                                              output_dir = NULL) {
  # Validate output_dir if saving
...
#'
#' @export
get_predictions_malaria <- function(data,
                            param_term,
                            model,
                            level){
...
#'
#' @export
contour_plot_malaria <- function(data,
                         param_term,
                         model,
                         level,
                         filter_year = NULL,
...
#'
#' @export
plot_rr_map_malaria <- function(combined_data,
                        model,
                        param_term = "tmax",
                        level = "District",
                        filter_year = NULL,
...
#'
#' @export
plot_relative_risk_malaria <- function(data,
                               model,
                               param_term,
                               level,
                               filter_year = NULL,
  ...
  build_plot <- function(pred, title) {
    ...
    ggplot2::ggplot(
      ...
      ggplot2::geom_hline(yintercept = 1, linetype = "dashed", color = "blue", linewidth = 0.5) +
...
#'
#' @export
attribution_calculation_malaria <- function(data,
                                    param_term,
                                    model,
                                    level,
                                    param_threshold = 1,
  ...
  compute_metrics_from_pred <- function(df, pred) {
    df <- df[!is.na(df[[param_term]]) & !is.na(df$Malaria) & !is.na(df$tot_pop), ]
...
#' Plot attributable metrics across spatial levels
#'
#' @description
#' Generates plots of selected attributable metrics (e.g., number, fraction, or rate)
#' from attribution results across spatial levels (country, region, or district).
#' Allows optional filtering by year and focus area, and supports saving output figures
#' as PDFs.
#'
#' @param attr_data A data frame of attribution results, typically from
#' `attribution_calculation()`, containing columns like year, region, district,
#' AR_Number, AR_Fraction, and AR_per_100k.
#' @param level Character. Spatial level for plotting. One of "country", "region", or "district".
#' @param metrics Character vector of one or more metrics to plot.
#' Options include "AR_Number", "AR_Fraction", and "AR_per_100k".
#' @param filter_year Optional integer or vector of integers to restrict plots to specific years.
#' Defaults to NULL (no filtering).
#' @param param_term A character vector or list containing parameter terms such
#' as "tmax" (maximum temperature) and "rainfall" (precipitation).
#' Default to "tmax".
#' @param save_fig Logical. If TRUE, saves plots as PDF files. Defaults to FALSE.
#' @param output_dir Optional string. Directory path to save PDF files if `save_fig = TRUE`.
#'
#' @return A named list of ggplot objects, grouped by metric.
#' Each metric may return one or more plots depending on the number of years and filtering.
#'
#' @export
plot_attribution_metric_malaria <- function(attr_data,
  ...
  title_lookup <- c(
    AR_per_100k = paste0("Malaria cases per 100,000 attributable to ", param_label, " (95% CI)"),
    AR_Fraction = paste0("Attributable Fraction (%) due to ", param_label, " (95% CI)"),
    AR_Number = paste0("Number of Malaria cases attributable to ", param_label, " (95% CI)")
  )
...
malaria_do_analysis <- function(health_data_path,
  ...
  # get combined data
  combined_data <- combine_health_climate_data_malaria(health_data_path,
                                               climate_data_path,
                                               map_path,
                                               region_col,
                                               district_col,
                                               ...
                                               output_dir)
  #plot time seris
  plot_Malaria<-plot_health_climate_timeseries_malaria(combined_data$data,
                                               param_term = "malaria",
                                               level = "country",
                                               filter_year = filter_year,
                                               save_fig = save_fig,
                                               output_dir = output_dir)
  plot_tmax<-plot_health_climate_timeseries_malaria(combined_data$data,
                                            param_term = "tmax",
                                            level = "country",
                                            filter_year = filter_year,
                                            save_fig = save_fig,
  ...
  plot_rainfall<-plot_health_climate_timeseries_malaria(combined_data$data,
                                                param_term = "rainfall",
                                                level = "country",
                                                filter_year = filter_year,
                                                save_fig = save_fig,
                                                output_dir = output_dir)
  # create base matrice
  basis <- set_cross_basis_malaria(combined_data$data)

  #check for multicolinearity
  VIF<- check_vif_malaria(combined_data$data, inla_param, basis_matrices_choices)
  # fitting the model
  inla_result <- run_inla_models_malaria(
    combined_data,
    basis_matrices_choices,
    inla_param,
    output_dir=output_dir,
  ...
  reff_plot_yearly <- plot_yearly_spatial_random_effect_malaria(
    combined_data,
    model=inla_result$model,
    output_dir=output_dir,
    save_fig=save_fig
  ...
  # contour plots
  contour_plot <- contour_plot_malaria(combined_data$data, param_term, model=inla_result$model,
                               level=level, output_dir=output_dir,
                               save_fig=save_fig, filter_year=filter_year)

  # rr map plots
  rr_map_plot <- plot_rr_map_malaria(
    combined_data,
    param_term,
    model=inla_result$model,
    level="district",
  ...
  # relative rist plot
  rr_data <- plot_relative_risk_malaria(
    combined_data$data,
  ...
  attr_frac_num <- attribution_calculation_malaria(
    combined_data$data,
    param_term=param_term,
    model=inla_result$model,
  ...
  plot_AR_num <-plot_attribution_metric_malaria(attr_data = attr_frac_num,
                                        param_term=param_term,
                                        level= level,
                                        metrics = "AR_Number",
                                        filter_year = filter_year,
  ...
  plot_AR_frac <-plot_attribution_metric_malaria(attr_data = attr_frac_num,
                                       param_term=param_term,
                                       level= level,
                                       metrics = "AR_Fraction",
                                       filter_year = filter_year,
  ...
  plot_AR_per_100k <-plot_attribution_metric_malaria(attr_data = attr_frac_num,
                                             param_term=param_term,
                                             level= level,
                                             filter_year = filter_year,
                                             metrics = "AR_per_100k",
  ...
  res <- list(plot_malaria, plot_tmax, plot_rainfall, inla_result, reff_plot_monthly,
              reff_plot_yearly, contour_plot, rr_map_plot,
              rr_plot, rr_df, attr_frac_num, plot_AR_num, plot_AR_frac,
              plot_AR_per_100k)
