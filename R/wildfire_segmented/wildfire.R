...
load_wildfire_data <- function(health_path,
  ...
  if (join_wildfire_data == FALSE) {
    ...
    # Placeholder for when join_wildfire_data == TRUE
    # You can add logic here to join with wildfire shapefiles or NetCDF data
    # Adding join with wildfire shapefile/netcdf data
    ...
    # message("Joining wildfire data is not yet implemented.")
    return(wildfire_data)
...
#'
#' @export
check_vif <- function(data, predictors, print_vif = TRUE) {
...
casecrossover_quasipoisson <- function(data,
  ...
  for (i in lags) {
    ...
    formula <- as.formula(paste("health_outcome ~ splines::ns(tmean, df = 6) +", i))
...
#' Relative risk with PM2.5 results.
#' @description Calculates how different PM 2.5 levels changes the Relative risk
#'
#' @param results Dataframe of relative risk and different wildfire-related PM2.5 levels
...
generate_rr_pm_overall <- function(relative_risk_overall,
                                   scale_factor_wildfire_pm,
                                   wildfire_lag = 0,
  ...
  rr_lower <- exp((coef_pm - 1.96 * se_pm) * (pm_vals / scale_factor_wildfire_pm))
  rr_upper <- exp((coef_pm + 1.96 * se_pm) * (pm_vals / scale_factor_wildfire_pm))
...
summarise_AF_AN <- function(data){
  ...
  data <- data %>%
    mutate(
      se_AN = (attributable_number_upper - attributable_number_lower) / (2 * z),
      var_AN = se_AN^2,
      se_AF = (attributable_fraction_upper - attributable_fraction_lower) / (2 * z),
      var_AF = se_AF^2
    )
  ...
  yearly_summary <- data %>%
    ...
    summarise(
      ...
      total_variance_AN = sum(var_AN, na.rm = TRUE),
      average_attributable_fraction = mean(attributable_fraction, na.rm = TRUE),
      variance_AF_mean = sum(var_AF, na.rm = TRUE) / n(),  # variance of the mean AF
      .groups = "drop"
    ...
    mutate(
      ...
      se_total_AN = sqrt(total_variance_AN),
      lower_ci_attributable_number = total_attributable_number - z * se_total_AN,
      upper_ci_attributable_number = total_attributable_number + z * se_total_AN,

      # Confidence intervals for average AF
      se_AF_mean = sqrt(variance_AF_mean),
      lower_ci_attributable_fraction = average_attributable_fraction - z * se_AF_mean,
      upper_ci_attributable_fraction = average_attributable_fraction + z * se_AF_mean,
